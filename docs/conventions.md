...
- Una feature no importa el `repo`, el `schema` ni las `routes` de otra: declara lo que necesita como **puerto en su propio `ports.ts`** y la raíz de composición le inyecta la implementación de la otra feature (ver `docs/architecture.md` §2 y §3.5).
...
- [ ] **Sin dependencias hacia adaptadores desde los servicios**: `service.ts`/`ports.ts` no importan `hono`, `drizzle-orm`, `better-auth`, `sharp`, `node:fs`, `./repo.ts`, `./schema.ts` ni nada de otra feature que no sea un puerto inyectado; ninguna ruta nueva usa `db.` directo; el repo cumple su puerto con `satisfies`; el caso de uso tiene su test con fakes.
