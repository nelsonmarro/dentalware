...
| 31 | **Reglas de rol y de estado del trabajo en `shared`, una sola fuente**: `CASE_WRITE_ROLES`, `STAGE_CHANGE_ROLES`, `ASSIGN_TECHNICIAN_ROLES`, `REMAKE_ROLES`, `canChangeStage`, `canAssignTechnician`, `canRemake`, `STAGE_CHANGE_BLOCKED_REASON`, `REASON_REQUIRED_FOR_ACTION`; las consumen `requireRole(...)` en las rutas, el servicio y la web. Las clasificaciones por acción o estado son `Record` exhaustivos | con las listas escritas a mano en tres capas, la web ofrecía acciones que la API rechazaba; un `Record` exhaustivo hace que una acción o estado nuevos no compilen hasta clasificarse (decidido el 2026-09-24, revisión final del PR 1) | vigente |
