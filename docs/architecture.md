...
export interface UnitOfWork {
  run<T>(fn: (repos: { cases: CasesRepository; tryins: TryinsRepository }) => Promise<T>): Promise<T>
}
...
**Adaptador de persistencia** (`repo.ts`) — repositorio Drizzle, con la misma factoría para `db` y para `tx`, lo que sustituyó al par `createCaseTx` / `createCase` de antes (hoy `createCasesRepo`/`drizzleUnitOfWork` en `features/cases/repo.ts:220,479`; desde la Iteración 3 la unidad de trabajo reconstruye sobre `tx` los dos repositorios transaccionales del trabajo, `cases` y `tryins`):
...
export const drizzleUnitOfWork = (db: Db): UnitOfWork => ({
  run: (fn) => db.transaction((tx) => fn({ cases: createCasesRepo(tx), tryins: createTryinsRepo(tx) })),
})
```
...
**`apps/web/src/components/data-grid/**`** (ronda de fixes del PR 1 de #53, hallazgo I-4 de la revisión final): la regla de aislamiento de la spec del DataGrid (§2.1 — el núcleo no importa `features/`, una feature no importa otra) pasa de estar solo verificada por revisión a estarlo también por `pnpm lint`, con el mismo mecanismo de `@typescript-eslint/no-restricted-imports` por `patterns` y sin plugin nuevo. Tres bloques, del más general al más específico (el último que matchea un archivo reemplaza, no fusiona, así que los dos específicos repiten el patrón `dataGridAppBoundary` del general junto al suyo, igual que ya hacía el bloque 3 de arriba con el 2): (a) `data-grid/**` completo prohíbe `@/features/*`, `@/features/**`, `@/routes/*`, `@/routes/**` y `@/lib/api` — DataGrid es transversal (`components/`), no depende de una feature de la app, de una ruta ni del cliente HTTP; (b) el núcleo (`use-data-grid.ts`, `data-grid.tsx`, `context.ts`, `define-columns.ts`, `types.ts`, `parts/**`, con `parts/**/*.test.{ts,tsx}` excluido) prohíbe además `./features/*`, `../features/*` y `**/data-grid/features/*` — `index.ts` queda fuera de este bloque a propósito: es el barrel que legítimamente re-exporta las factorías de `features/` (única vía, igual que un `ports.ts` en la API); (c) cada módulo de `features/*.{ts,tsx}` (con sus propios `*.test.{ts,tsx}` excluidos) prohíbe importar el núcleo (`../use-data-grid`, `../data-grid`) y los archivos de entrada de sus hermanos (`./sorting`, `./pagination`, `./filtering`, `./resizing`, `./grouping`, `./advanced-filter`, `./pinning`, `./url-state`, con y sin extensión) más, desde la Tarea 10 (#53), `**/data-grid/features/*` — las rutas relativas (`./resizing`) no cubrían el alias absoluto entre hermanas (`@/components/data-grid/features/sorting`), que sí importaría sin que el lint lo viera; sus propios sub-archivos privados (p. ej. `filtering-column-filter.tsx`) no están en la lista y se pueden importar, porque solo se referencian con ruta relativa (`./nombre-*`), que no matchea ese patrón. Los tests de `features/*` y de `parts/*` quedan fuera de sus bloques respectivos (mismo criterio que el resto de `apps/web`, arriba): montan el núcleo real o una feature real para probar comportamiento, no son parte de la frontera que se protege. Pruebas negativas (introducir el import prohibido, ver el error de ESLint, revertir) documentadas en el reporte de esa ronda de fixes y, para el patrón `**/data-grid/features/*`, en el reporte de la Tarea 10.
...
|---|---|---|---|
| `cases` | **migrada** (PR A) | `ports.ts` (`CasesRepository`, `TryinsRepository`, `AttachmentsQuery`, `StagesQuery`, `UsersQuery`, `UnitOfWork`; el reloj llega como `Clock` de `lib/clock.ts`), `service.ts` con enmascarado (`stripPrices`/`maskPriceEvents`), readiness y, desde la Iteración 3, el ciclo de vida (`action`, `changeStage`, `assignTechnician`, `createRemake`, `technicians`), `errors.ts` (`CaseInputError`, `CaseStateError`, `CaseNotFoundError`, `CaseForbiddenError`), `repo.ts` (`createCasesRepo(db \| tx) satisfies CasesRepository`, `createTryinsRepo`, `createUsersQuery`, `drizzleUnitOfWork`), `fakes.ts`, tests de servicio con fakes (`service.test.ts`, 61 al cerrar el PR 1 de la Iteración 3) | hecho |
...
| `users` | sin `repo.ts`: Drizzle directo en la ruta (`routes.ts:47,65,99,120`); Better Auth acoplado al handler. Desde la Iteración 3, `cases` lee los técnicos activos por su propio puerto `UsersQuery` (`createUsersQuery` en `cases/repo.ts`, lectura de solo lectura por ADR 24), sin tocar esta feature | `repo.ts` + `ports.ts` (`UsersRepository`, `AccountProvider`) y `service.ts` con las reglas propias (nadie se bloquea ni se degrada a sí mismo, borrado lógico) | boy-scout: al primer cambio funcional; a más tardar Iteración 5 |
...
| `clinics`, `doctors`, `stages`, `lab-settings` | CRUD limpio, sin reglas | nada: se acogen a la excepción de §3.4; solo declarar el puerto cuando otra feature los consuma. `stages` ya lo consume `cases` como `StagesQuery` (`app.ts:54`, un objeto literal sobre `listStages`) | — |
...
| 27 | **Una sola ruta para las transiciones de estado** (`POST /api/trabajos/:id/acciones` con `{ accion, motivo }`), con `requireRole(...CASE_ACTION_ROLES)` en la ruta —la **unión** de roles de `CASE_TRANSITIONS`, derivada en `shared`, no escrita a mano— y `canPerform(role, accion)` en el servicio | el rol permitido depende de la acción (el técnico finaliza, el mensajero marca enviado/entregado), así que un guardián por ruta obligaría a una ruta por acción; el de la ruta mantiene el 403 uniforme antes del validador y el del servicio es la regla real, que sobrevive a que alguien toque la ruta (decidido el 2026-09-21, Tarea 5) | vigente |
| 28 | **La fase de producción es un recurso aparte del estado** (`PUT /api/trabajos/:id/fase`, `direccion` avanzar/retroceder con motivo al retroceder) y solo se cambia en `en_proceso` (`canChangeStage` en `shared`, lista blanca); «Finalizar» es una transición de estado y la sirve solo la barra de acciones | fase y estado evolucionan a ritmos distintos (un trabajo pasa por varias fases sin cambiar de estado); con lista blanca, un estado nuevo nace bloqueado en vez de colarse; un solo dueño de «Finalizar» evita dos botones idénticos en la misma pantalla (decidido el 2026-09-21/23, Tareas 6 y 9) | vigente |
| 29 | **`GET /api/trabajos/tecnicos` vive en el router de trabajos** y lee `users` por el puerto `UsersQuery` de `cases` (extensión del ADR 24), devolviendo solo `{ id, name }` a quien puede asignar | recepción asigna técnico pero `GET /api/usuarios` es solo de admin; aflojar ese router dispararía la migración boy-scout de `users` (§3.6), y recepción no debe recibir correos, roles ni estado de baneo para rellenar un selector (decidido el 2026-09-22, Tarea 9) | vigente |
| 30 | **Sin feriados en la fecha comprometida**: `promised_date` = recepción + `turnaround_days` en días hábiles (lunes a viernes) con `addBusinessDays(inicio, días, [])` | el MVP no tiene calendario de feriados (Ecuador los mueve por decreto cada año) y la fecha es orientativa para la clínica; cuando exista un calendario se inyecta como tercer argumento sin tocar la regla (decidido el 2026-09-19, plan de la Iteración 3) | vigente |
| 31 | **Reglas de rol y de estado del trabajo en `shared`, una sola fuente**: `CASE_WRITE_ROLES`, `STAGE_CHANGE_ROLES`, `ASSIGN_TECHNICIAN_ROLES`, `REMAKE_ROLES`, `CASE_ACTION_ROLES`, `canChangeStage`, `canAssignTechnician`, `canRemake`, `STAGE_CHANGE_BLOCKED_REASON`; cada ruta usa **su** constante en `requireRole(...)` (aunque hoy `ASSIGN_TECHNICIAN_ROLES` y `REMAKE_ROLES` valgan lo mismo que `CASE_WRITE_ROLES`), y el servicio y la web consumen las mismas. `ACTIONS_REQUIRING_REASON` se deriva de un `Record<CaseAction, boolean>` exhaustivo. Las clasificaciones por acción o estado son `Record` exhaustivos | con las listas escritas a mano en tres capas, la web ofrecía acciones que la API rechazaba; un `Record` exhaustivo hace que una acción o estado nuevos no compilen hasta clasificarse (decidido el 2026-09-24, revisión final del PR 1) | vigente |
