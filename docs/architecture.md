...
1. `no-restricted-globals: fetch` fuera de `**/api.ts`, con dos excepciones (`ignores`): `features/cases/attachments-api.ts` (sube con `fetch` directo porque `hc` no tipa `form`, documentado en el propio archivo) y `test/**` (harness de pruebas).
...
**Excepción implícita — tests y `fakes.ts`**: ninguno de los bloques anteriores incluye `*.test.ts`/`*.test.tsx` ni `fakes.ts` en sus `files`, así que las pruebas de integración de rutas pueden seguir usando `db`/`createApp` reales y `fakes.ts` puede construir implementaciones en memoria de los puertos sin disparar ninguna regla; es una ausencia deliberada de cobertura, no una excepción con `ignores`.
