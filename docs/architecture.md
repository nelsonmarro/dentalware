...
| `cases/import` | caso de uso (`importCases`, `import.ts:93`) y adaptador HTTP (`importRoutes`, `import.ts:77`) en el mismo archivo; lee `schema.ts` de clinics/doctors/products (`import.ts:16-18`); `new Date()` dentro de la ruta (`import.ts:121`) | separar en `import.service.ts` + `import.routes.ts`; puertos `ClinicsQuery`/`DoctorsQuery`/`ProductsQuery`; `Clock` inyectado | Iteración 3, con `cases` |
...
| `deliveries` (It. 4), `payments`/`accounts` (It. 5), `notifications` (It. 6) | no existen | nacen ya con `ports.ts` + `service.ts` + tests con fakes (obligatorio) | al construirlas |
...
- **Nueva integración externa** (S3/R2, WhatsApp, correo, SRI): interfaz en el `ports.ts` de la feature que la usa (o en `lib/` si es transversal), implementación en `lib/` o `features/<f>/<adaptador>.ts`, inyectada en `createApp`, fake en tests. Ninguna feature cambia.
